...
public class IdUtil {
	 ...
	 *            L'id a analysé
	 * 
	 * @return Si le jour est trouve, une TreeMap avec, comme clé, les constante
	 *         KEY de cette classe (YEAR, MONTH et WEEK)
	 * 
	...
	public static GregorianCalendar getDayFromId(int id) {
		for (int k = YEAR_START; k <= YEAR_END; k++) {
			for (int i = 0; i <= 11; i++) {
				for (int j = 1; j <= 31; j++) {
					if (id == k * STRING_YEAR.hashCode() + i
							* STRING_MONTH.hashCode() + j
							* STRING_DAY.hashCode()) {
						final GregorianCalendar cal = new GregorianCalendar(k,
								i, j, 0, 0, 0);
						return cal;
					}
				}
	 ...
	 * 
	 * @param year
	 *            L'année MonthDisplayHelper.getYear() retourne cette valeur...
	 * 
	 * @param month
	 *            De 0 à 11 -> MonthDisplayHelper.getMonth() retourne cette
	 *            valeur...
	 * 
	 * @param day
	 *            de 1 à 31
	...
	public static int getDayId(int year, int month, int day) {
		return (year) * STRING_YEAR.hashCode() + (month)
				* STRING_MONTH.hashCode() + day * STRING_DAY.hashCode();
	}
	 ...
	 *            L'id a analysé
	 * 
	 * @return Si la semaine est trouve, une TreeMap avec, comme clé, les
	 *         constante KEY de cette classe (YEAR et WEEK)
	 * 
	 *         Sinon, si le jour n'est pas trouvé, retourne null
	 */
